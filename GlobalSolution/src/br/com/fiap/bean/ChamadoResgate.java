...
public class ChamadoResgate extends Ong {
    ...
    private int calcularEmergencia() {
        if (getSituacaoVitima().equalsIgnoreCase("machucado seriamente")) {
            return 3;
        } else if (getSituacaoVitima().equalsIgnoreCase("machucado levemente")) {
            return 2;
        }
        return 1;
    }
    ...
    // Método privado para gerar resumo (usado internamente)
    private String gerarResumoChamado() {
        ...
        return String.format(
            ...
            "Tipo: %s\nSituação: %s\nRegião: %s\n" +
            "Data: %s\nNível emergência: %d\nONG: %s",
            idChamado, tipoChamado, situacaoVitima, regiao,
            dataHora.format(formatter), nivelEmergencia, getNome()
        );
    }
}
